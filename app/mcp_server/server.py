...
mcp = FastMCP(
    ...
    instructions=(
        "Сервер библиотеки: поиск книг и доставка ВЫБРАННОЙ пользователем книги на e-mail.\n"
        "\n"
        "Главное правило выбора книги:\n"
        "- search_books может вернуть МНОГО книг, в т.ч. несколько вариантов одной и той же "
        "книги (разные издания/издательства, с иллюстрациями и без, разные форматы и размер).\n"
        "- Какую именно книгу взять — решает КОНЕЧНЫЙ ПОЛЬЗОВАТЕЛЬ. Агент НЕ выбирает книгу "
        "сам: он показывает пользователю найденные варианты, кратко поясняет отличия и просит "
        "выбрать один. Агент может лишь ПОРЕКОМЕНДОВАТЬ вариант (по предпочтениям пользователя "
        "или здравому смыслу), но окончательный выбор — всегда за пользователем.\n"
        "- 'РОВНО ОДНА книга' / 'один book_id' в export_book_to_s3 и send_book_to_email — это "
        "ТЕХНИЧЕСКОЕ ограничение инструментов (за раз обрабатывается одна книга), а НЕ право "
        "выбрать книгу за пользователя и не повод пропускать его подтверждение.\n"
        "\n"
        "Порядок вызовов (шаги нельзя пропускать или менять местами):\n"
        "1. search_books — найди книги по запросу пользователя. 'too_many_results' — уточни "
        "запрос и повтори; 'no_results' — упрости/измени запрос; 'validation_error' — не задан "
        "ни один параметр поиска.\n"
        "2. Пользователь (через агента) выбирает РОВНО ОДНУ книгу из результатов. До явного "
        "выбора пользователя НЕ вызывай export_book_to_s3 и send_book_to_email.\n"
        "3. export_book_to_s3 — выгрузи выбранную книгу в S3 (один book_id). Возьми bucket и key.\n"
        "4. send_book_to_email — отправь книгу на e-mail (bucket и file_key из шага 3). ТОЛЬКО "
        "после успешного export_book_to_s3, иначе вернётся статус 'not_in_s3'.\n"
    ),
)
...
@mcp.tool(
    ...
    description=(
        ...
        "Может вернуть НЕСКОЛЬКО книг, в том числе разные варианты одной и той же книги "
        "(издания/издательства, с иллюстрациями и без, форматы, размер).\n"
        "Выбор конкретной книги из результатов — за ПОЛЬЗОВАТЕЛЕМ (через агента), а не за "
        "моделью: покажи варианты пользователю, поясни отличия и дай выбрать один; можно "
        "порекомендовать вариант, но не выбирать за пользователя. К export_book_to_s3 "
        "переходи только с тем book_id, который пользователь явно выбрал.\n"
        "Статусы ответа:\n"
        "- 'ok' — в books список найденных книг (одна или несколько); покажи их пользователю.\n"
...
@mcp.tool(
    ...
    description=(
        "Шаг 2 из 3. Выгружает файл ОДНОЙ книги в S3/MinIO по её id. "
        "За раз обрабатывается ровно одна книга (техническое ограничение). "
        "book_id — это та книга из результата search_books, которую ЯВНО выбрал "
        "пользователь; модель не выбирает книгу сама.\n"
...
async def export_book_to_s3(
    book_id: Annotated[
        ...
        Field(
            description=(
                "ID одной книги из результата search_books — той, которую явно "
                "выбрал пользователь"
            ),
            ge=1,
        ),
...
@mcp.tool(
    ...
    description=(
        ...
        "Вызывать ТОЛЬКО после успешного export_book_to_s3 и только для той книги, "
        "которую пользователь выбрал и подтвердил отправку.\n"
